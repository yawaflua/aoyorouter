...
func (p *ProviderCline) AddProviderConfig(ctx context.Context, cfg *config.Config, provider *models.Provider) {
	...
	if idx := slices.IndexFunc(cfg.OpenAICompatibility, func(item config.OpenAICompatibility) bool {
		return item.Name == p.GetOAuthDefinition().Provider
	}); idx >= 0 {
		cfg.OpenAICompatibility[idx].APIKeyEntries = append(cfg.OpenAICompatibility[idx].APIKeyEntries, config.OpenAICompatibilityAPIKey{
			APIKey: provider.ClientSecret,
		})
	} else {
		var models []config.OpenAICompatibilityModel = []config.OpenAICompatibilityModel{
			{Name: "cline-pass/glm-5.3", DisplayName: "GLM-5.3"},
			{Name: "cline-pass/glm-5.2", DisplayName: "GLM-5.2"},
			{Name: "cline-pass/kimi-k3", DisplayName: "Kimi-K3"},
			{Name: "cline-pass/kimi-k2.7-code", DisplayName: "Kimi-K2.7-Code"},
			{Name: "cline-pass/kimi-k2.6", DisplayName: "Kimi-K2.6"},
			{Name: "cline-pass/deepseek-v4-pro", DisplayName: "DeepSeek-V4-Pro"},
			{Name: "cline-pass/deepseek-v4-flash", DisplayName: "DeepSeek-V4-Flash"},
			{Name: "cline-pass/mimo-v2.5", DisplayName: "MIMO-V2.5"},
			{Name: "cline-pass/mimo-v2.5-pro", DisplayName: "MIMO-V2.5-Pro"},
			{Name: "cline-pass/minimax-m3", DisplayName: "Minimax-M3"},
			{Name: "cline-pass/qwen3.8-max", DisplayName: "Qwen3.8-Max"},
			{Name: "cline-pass/qwen3.7-max", DisplayName: "Qwen3.7-Max"},
			{Name: "cline-pass/qwen3.7-plus", DisplayName: "Qwen3.7-Plus"},
			{Name: "minimax/minimax-m2.5", DisplayName: "Cline/Minimax-M2.5"},
		}
		entry := config.OpenAICompatibility{
			Name:          "cline",
			Models:        models,
			BaseURL:       provider.BaseUrl,
			APIKeyEntries: []config.OpenAICompatibilityAPIKey{{APIKey: provider.ClientSecret}},
		}
